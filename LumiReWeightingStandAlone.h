  ...
  LumiReWeighting( std::string generatedFile,
    ...
    generatedFileName_( generatedFile),
      ...
      DataHistName_     ( DataHistName )
      {
        ...
        if ( MC_distr_temp->GetNbinsX() > Data_distr_->GetNbinsX() ) {
          MC_distr_ = new TH1F(  *(static_cast<TH1F*>(Data_distr_->Clone())) );
          for(int ibin = 1; ibin<Data_distr_->GetNbinsX()+1; ++ibin){
            MC_distr_->SetBinContent( ibin, MC_distr_temp->GetBinContent(ibin) );
          }
        }
        else {MC_distr_ = MC_distr_temp;}
